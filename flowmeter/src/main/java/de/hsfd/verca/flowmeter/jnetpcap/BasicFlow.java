package de.hsfd.verca.flowmeter.jnetpcap;
...
public class BasicFlow {
    ...
    void detectUpdateSubflows(BasicPacketInfo packet) {
        ...
        if ((packet.getTimeStamp() - (sfLastPacketTS) / (double) 1000000) > 1.0) {
            ...
            updateActiveIdleTime(packet.getTimeStamp() - sfLastPacketTS, FlowConfig.activityTimeout);
            sfAcHelper = packet.getTimeStamp();
        }
    ...
    public void endActiveIdleTime(long currentTime, long threshold, long flowTimeOut, boolean isFlagEnd) {
        // System.out.println(this.endActiveTime - this.startActiveTime);
    ...
    public double ipToDouble(String ipAddress) {
    ...
    public ArrayList<Double> dumpFlowBasedFeaturesTransformed() {
        ArrayList<Double> dumpList = new ArrayList<>();

        //dumpList.add(flowId);                                           //1
        dumpList.add(ipToDouble(FormatUtils.ip(src)));                    //2
        dumpList.add((double) getSrcPort());                               //3
        dumpList.add(ipToDouble(FormatUtils.ip(dst)));                    //4
        dumpList.add((double) getDstPort());                               //5
        dumpList.add((double) getProtocol());                              //6
        dumpList.add((double) flowStartTime);                              //7

        long flowDuration = flowLastSeen - flowStartTime;
        dumpList.add((double) flowDuration);                               //8

        dumpList.add((double) fwdPktStats.getN());                         //9
        dumpList.add((double) bwdPktStats.getN());                         //10
        dumpList.add(fwdPktStats.getSum());                               //11
        dumpList.add(bwdPktStats.getSum());                               //12

        if (fwdPktStats.getN() > 0L) {
            dumpList.add(fwdPktStats.getMax());                           //13
            dumpList.add(fwdPktStats.getMin());                           //14
            dumpList.add(fwdPktStats.getMean());                          //15
            dumpList.add(fwdPktStats.getStandardDeviation());             //16
        } else {
            dumpList.add(0.0d);
            dumpList.add(0.0d);
            dumpList.add(0.0d);
        ...
        if (bwdPktStats.getN() > 0L) {
            dumpList.add(bwdPktStats.getMax());                           //17
            dumpList.add(bwdPktStats.getMin());                           //18
            dumpList.add(bwdPktStats.getMean());                          //19
            dumpList.add(bwdPktStats.getStandardDeviation());             //20
        } else {
            dumpList.add(0.0d);
            dumpList.add(0.0d);
            dumpList.add(0.0d);
        ...
        dumpList.add(((double) (forwardBytes + backwardBytes)) / ((double) flowDuration / 1000000L));       //21
        dumpList.add(((double) packetCount()) / ((double) flowDuration / 1000000L));                        //22
        dumpList.add(flowIAT.getMean());                                                                    //23
        dumpList.add(flowIAT.getStandardDeviation());                                                       //24
        dumpList.add(flowIAT.getMax());                                                                     //25
        dumpList.add(flowIAT.getMin());                                                                     //26

        if (this.forward.size() > 1) {
            dumpList.add(forwardIAT.getSum());                                                              //27
            dumpList.add(forwardIAT.getMean());                                                             //28
            dumpList.add(forwardIAT.getStandardDeviation());                                                //29
            dumpList.add(forwardIAT.getMax());                                                              //30
            dumpList.add(forwardIAT.getMin());                                                              //31
        ...
        if (this.backward.size() > 1) {
            dumpList.add(backwardIAT.getSum());                                                            //32
            dumpList.add(backwardIAT.getMean());                                                           //33
            dumpList.add(backwardIAT.getStandardDeviation());                                              //34
            dumpList.add(backwardIAT.getMax());                                                            //35
            dumpList.add(backwardIAT.getMin());                                                            //36
        } else {
            dumpList.add(0.0d);
            dumpList.add(0.0d);
            dumpList.add(0.0d);
        ...
        //dumpList.add((double) fPSH_cnt);                                                                    //37
        //dumpList.add((double) bPSH_cnt);                                                                    //38
        //dumpList.add((double) fURG_cnt);                                                                    //39
        //dumpList.add((double) bURG_cnt);                                                                    //40

        dumpList.add((double) fHeaderBytes);                                                                //41
        dumpList.add((double) bHeaderBytes);                                                                //42
        dumpList.add(getfPktsPerSecond());                                                                 //43
        dumpList.add(getbPktsPerSecond());                                                                 //44


        if (this.forward.size() > 0 || this.backward.size() > 0) {
            dumpList.add(flowLengthStats.getMin());                                                        //45
            dumpList.add(flowLengthStats.getMax());                                                        //46
            dumpList.add(flowLengthStats.getMean());                                                       //47
            dumpList.add(flowLengthStats.getStandardDeviation());                                          //48
            dumpList.add(flowLengthStats.getVariance());                                                   //49
        ...
        dumpList.add((double) flagCounts.get("FIN").value);                                                 //50
        dumpList.add((double) flagCounts.get("SYN").value);                                                 //51
        dumpList.add((double) flagCounts.get("RST").value);                                                 //52
        dumpList.add((double) flagCounts.get("PSH").value);                                                 //53
        dumpList.add((double) flagCounts.get("ACK").value);                                                 //54
        dumpList.add((double) flagCounts.get("URG").value);                                                 //55
        dumpList.add((double) flagCounts.get("CWR").value);                                                 //56
        dumpList.add((double) flagCounts.get("ECE").value);                                                 //57

        dumpList.add(getDownUpRatio());                                                                    //58
        dumpList.add(getAvgPacketSize());                                                                  //59
        dumpList.add(fAvgSegmentSize());                                                                   //60
        dumpList.add(bAvgSegmentSize());                                                                   //61

        dumpList.add((double) fAvgBytesPerBulk());                                                         //62
        dumpList.add((double) fAvgPacketsPerBulk());                                                       //63
        dumpList.add((double) fAvgBulkRate());                                                             //64
        dumpList.add((double) fAvgBytesPerBulk());                                                         //65
        dumpList.add((double) bAvgPacketsPerBulk());                                                       //66
        dumpList.add((double) bAvgBulkRate());                                                             //67

        dumpList.add((double) getSflow_fpackets());                                                        //68
        dumpList.add((double) getSflow_fbytes());                                                          //79
        dumpList.add((double) getSflow_bpackets());                                                        //71
        dumpList.add((double) getSflow_bbytes());                                                          //72

        dumpList.add((double) Init_Win_bytes_forward);                                                     //73
        dumpList.add((double) Init_Win_bytes_backward);                                                    //74
        dumpList.add((double) Act_data_pkt_forward);                                                       //75
        dumpList.add((double) min_seg_size_forward);                                                       //76


        if (this.flowActive.getN() > 0) {
            dumpList.add(flowActive.getMean());                                                            //77
            dumpList.add(flowActive.getStandardDeviation());                                               //78
            dumpList.add(flowActive.getMax());                                                             //79
            dumpList.add(flowActive.getMin());                                                             //80
        } else {
            dumpList.add(0.0d);
            dumpList.add(0.0d);
            dumpList.add(0.0d);
        ...
        if (this.flowIdle.getN() > 0) {
            dumpList.add(flowIdle.getMean());                                                              //81
            dumpList.add(flowIdle.getStandardDeviation());                                                 //82
            dumpList.add(flowIdle.getMax());                                                               //83
            dumpList.add(flowIdle.getMin());                                                               //84
        } else {
            dumpList.add(0.0d);
            dumpList.add(0.0d);
            dumpList.add(0.0d);
